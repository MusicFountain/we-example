...
Page({
    ...
    wx.uploadFile({
      ...
      success: function (res) {
        ...
        if (data.data.errormsg != "OK") {
          ...
          // 若识别结果为"Musical Fountain "，则向oneNET请求数据并绘图
          if (str == "Musical Fountain ") {
            //从oneNET请求我们的Wi-Fi气象站的数据
            const requestTask = wx.request({
              url: 'https://api.heclouds.com/devices/503186891/datapoints?datastream_id=Light,Temperature,Humidity&limit=10',
              header: {
                'content-type': 'application/json',  
                'api-key': 'RxSmIMxd8xBHR8DWO=zotjzgQuw='
              },
            ...
            //若识别结果不是"Hello World from E128"，则给出提示
            wx.showModal({
              title: '提示',
              content: '图片中的文字不是"Musical Fountain "',
              success: function (res) {
                if (res.confirm) {
                  console.log('用户点击确定')
                } else if (res.cancel) {
  ...
  onLoad: function () {
    wx.showModal({
      title: '验证口令',
      content: '请上传一张写有‘Musical Fountain ’ 的图片，识别成功则绘制天气曲线。',
      success: function (res) {
        if (res.confirm) {
          console.log('用户点击确定')
        } else if (res.cancel) {
